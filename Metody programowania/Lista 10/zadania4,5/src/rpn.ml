...
type cmd =
  ...
  | PushPair
  | PushUnit
  | PushVar of Ast.ident (* Zadanie 4 *)
  | Fst
  | Snd
  | IsPair
  | Binop    of Ast.bop
...
(* Zadanie 4 *)
(* Ewaluator dla RPN ze zmiennymi i środowiskiem*)
...
let rec eval (s : I.value list) (p : prog) (env : I.env) : I.value =
  ...
  | [], _   -> failwith "error!"
  | (PushInt n :: p'), _ ->
      eval (I.VInt n :: s) p' env
  | (PushBool b :: p'), _ ->
      eval (I.VBool b :: s) p' env
  | (Binop op :: p'), (v2 :: v1 :: s') ->
      eval (I.eval_op op v1 v2 :: s') p' env
  | (CndJmp (t,_) :: p'), (I.VBool true :: s') ->
      eval s' (t @ p') env
  | (CndJmp (_,e) :: p'), (I.VBool false :: s') ->
      eval s' (e @ p') env
  | (PushPair :: p'), (v2 :: v1 :: s') ->
      eval (I.VPair (v1, v2) :: s') p' env
  | (Fst :: p'), (I.VPair (v1,_) :: s') ->
      eval (v1 :: s') p' env
  | (Snd :: p'), (I.VPair (_,v2) :: s') ->
      eval (v2 :: s') p' env
  | (PushUnit :: p'), (s') ->
      eval (I.VUnit :: s') p' env
      (* Zadanie 4 *)
  | (PushVar x :: p'), _ ->
      (match I.M.find_opt x env with
      | Some v -> eval (v :: s) p' env
...
let stack_size (p : prog) : int =
  let rec aux p curr =
    ...
    | (PushVar _ :: p') -> max (aux p' (curr + 1)) (curr + 1)    
