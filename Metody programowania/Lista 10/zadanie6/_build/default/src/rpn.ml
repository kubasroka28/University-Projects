...
let rec emit_cmd (c : cmd) : string =
  ...
  | Binop op ->
    (match op with
    | Eq -> (* uruchamiam funkcję strukturalnego porównania, przekazuję wskaźniki na tagi obiektów, które porównuję *)
      alloc_pop
        [emit_bop_res_tag op;
         ("structural_comparison(stack[stack_ptr-1], stack[stack_ptr])")]
        2      
    | _ -> (* Bez zmian *)
      alloc_pop
        [emit_bop_res_tag op;
