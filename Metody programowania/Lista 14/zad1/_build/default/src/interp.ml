...
let eval_op (op : bop) : value -> value -> value comp =
  ...
  (* Zadanie 1 *)
let eval_uop (op : uop) (x : ident) : value comp =
  fun h ->
    match fst ((deref x) h) with
    | VInt x_val ->
      (match op with
      | Preinc  -> (VInt(x_val + 1), snd (assgn x (VInt(x_val + 1)) h))
      | Postinc -> (VInt(x_val),     snd (assgn x (VInt(x_val + 1)) h))
      | Predec  -> (VInt(x_val - 1), snd (assgn x (VInt(x_val - 1)) h))
      | Postdec -> (VInt(x_val),     snd (assgn x (VInt(x_val - 1)) h)))
    | _ -> failwith "type error"   
