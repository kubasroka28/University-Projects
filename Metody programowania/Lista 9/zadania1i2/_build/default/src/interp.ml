...
let rec show_value v =
  ...
  | VRef _ -> "<ref>"
  | VBreak | VContinue -> "<break/continue" (* Zadanie 2*)
...
let rec eval_env (env : env) (e : expr) : value comp =
  ...
  (* Zadanie 1 *)
  | While (c, e) ->
    let rec whileloop () : value comp =
      let* cond = eval_env env c in
      match cond with
      | VBool true ->
          (fun h ->
            try
              let (_, h') = eval_env env e h in
              whileloop () h'
            with
            | BreakExc -> (VUnit, h)
            | ContinueExc -> whileloop () h)
  ...
  (*| While (c, e) -> 
      let rec whileloop () =
        ...
        (match cond with
        | VBool true ->
            (try eval_env env e with
            | BreakExc -> return VUnit
            | ContinueExc -> whileloop ()
            | _ -> whileloop ())
        | VBool false -> return VUnit
        | _ -> failwith "not a condition")
      in whileloop ()*)
  ...
  (*| While (c, e) -> 
      let* cond = eval_env env c in
        (match cond with
        | VBool true -> 
          (try (let* _ = eval_env env e in eval_env env (While (c, e))) with
          | BreakExc -> return VUnit
          | ContinueExc -> eval_env env (While (c, e))
          )
          ...
          (match v with
          | VBreak -> return VUnit
          | VContinue -> eval_env env (While (c, e))
          | _ -> eval_env env (While (c, e))
          )*)
        | VBool false -> return VUnit
        | _ -> failwith "not a condition")*)
  (* Zadanie 2 *)
  | Break -> raise BreakExc
  | Continue -> raise ContinueExc
  (* Dla siebie *)
  | PrintInt x ->
      let* v = eval_env env x in
      (match v with
      | VInt i -> print_int i; return VUnit       
      | _ -> failwith "type error"
      )
