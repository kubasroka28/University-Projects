...
let insert (x : 'a) (sgt : 'a sgtree) : 'a sgtree =
    ...
    let is_balanced t : bool=
        ...
        | Node (l, _, r) ->
            ...
            let sum = l_size + 1 + r_size + 1 (*tymczasowe + 1, ktore dolicza nowo dodany wierzcholek, na liscie path sa drzewa, ktore nie maja go dodanego wiec trzeba to naprawic*) in
    ...
    in 
    let rec find_with_path x t (path_length : int) (path : 'a tree list) : int * 'a tree list =
      match t with
      | Leaf -> failwith "impossible case"
      | Node (l, v, r) ->
          if x = v then (path_length, path)
          else if x < v then find_with_path x l (path_length + 1) (t :: path)
          else find_with_path x r (path_length + 1) (t :: path)
    in *)
